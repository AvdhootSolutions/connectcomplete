...
<div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>Assign Executive Category </h1>
          </div>
          <div class="col-sm-6">
...
@section('content')
  <!-- Default box -->
      <div class="card card-primary">
              ...
              {!! Form::open(array(
                            'route' => 'assigncategory.store', 
                            'method'=>'POST', 
                            'id'=>'productlist-form',
                            'files'=> true
                    )) !!}
                    @csrf
              <input type="hidden" name="executive_id" value="{{request()->route('id')}}">  
              <div class="card-body">
                   <div class="row">
      ...
      <div class="card card-primary">
              ...
              <div class="card-body">
                   <table id="example1" class="table table-bordered table-striped">
                     ...
                     <tbody>
                      @if($data['executiveCategory']!="")
                      @foreach($data['executiveCategory'] as $key =>$executiveCategory)
                       <tr>
                         <td>{{$key+1}}</td>
                         <td>@if($executiveCategory->category!="") {{$executiveCategory->category->category_name}} @endif</td>
                         <td>@if($executiveCategory->subcategory!="") {{$executiveCategory->subcategory->subcategory_name}} @endif</td>
                         <td>
                           <form action="{{ url('/executiveCategoryDelete', ['id' => $executiveCategory->id]) }}" method="post" id="delete_form{{$executiveCategory->id}}">
                          <button  data-toggle="modal" data-target="#modal-danger{{ $executiveCategory->id}}"  class="btn btn-danger btn-xs" type="button"><i class="fa fa-trash" aria-hidden="true"></i></button>
                              @method('delete')
                              @csrf
                          </form>
                          <!-- Delete Modal -->
                          <div class="modal modal-danger fade" id="modal-danger{{ $executiveCategory->id}}">
                            <div class="modal-dialog">
                              <div class="modal-content">
                                ...
                                <div class="modal-footer">
                                  <button type="button" class="btn btn-outline pull-left" onclick="deleteItem('{{ $executiveCategory->id }}')">Ok</button>
