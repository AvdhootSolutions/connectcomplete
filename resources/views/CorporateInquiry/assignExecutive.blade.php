...
<div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>Assign Corporate Inquiry To Executive Member </h1>
          </div>
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="{{route('home')}}">Home</a></li>
              <li class="breadcrumb-item active">Assign Corporate Inquiry To Executive Member </li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
  ...
  <!-- Default box -->
      <div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title">Assign Corporate Inquiry To Executive Member</h3>
              </div>
              ...
              {!! Form::open(array(
                            'route' => 'storeAssignCorporateExecutiveinquiry', 
                            'method'=>'POST', 
                            'id'=>'productlist-form',
                            'files'=> true
                    )) !!}
      ...
      <div class="card card-primary">
              <div class="card-header" style="background-color: #001f3f">
                <h3 class="card-title">List Of Assigned Crew Members </h3>
              </div>
              <!-- /.card-header -->
              <!-- form start -->  
              <div class="card-body">
                   ...
                   <table id="example1" class="table table-bordered table-striped">
                     ...
                     <tbody>
                      @if($data['execitveInquiry']!="")
                      @foreach($data['execitveInquiry'] as $key =>$execitveInquiry)
                      
                       <tr>
                          <td>{{$key+1}}</td>
                          <td>@if($execitveInquiry->executives!=""){{ $execitveInquiry->executives->name }}@endif</td>
                          <td>
                           <form action="{{ url('/deleteCorporateExecutiveInquiry', ['id' => $execitveInquiry->id]) }}" method="post" id="delete_form{{$execitveInquiry->id}}">
                              

                          <button  data-toggle="modal" data-target="#modal-danger{{ $execitveInquiry->id}}"  class="btn btn-danger btn-xs" type="button"><i class="fa fa-trash" aria-hidden="true"></i></button>
                              @method('delete')
                              @csrf
                          </form>
                          <!-- Delete Modal -->
                          <div class="modal modal-danger fade" id="modal-danger{{ $execitveInquiry->id}}">
                            <div class="modal-dialog">
                              <div class="modal-content">
                                ...
                                <div class="modal-footer">
                                  <button type="button" class="btn btn-outline pull-left" onclick="deleteItem('{{ $execitveInquiry->id }}')">Ok</button>
